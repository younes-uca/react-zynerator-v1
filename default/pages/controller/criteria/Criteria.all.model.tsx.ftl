...
<#assign numberTypes = ["BigDecimal","Double"]>
 ...
 constructor(
   <#if pojo.id??>
   public ${pojo.id.name?uncap_first}: number = 0,
   </#if>
   <#list pojo.fieldsSimple as fieldSimple>
   <#if fieldSimple.id == false>
   <#if fieldSimple.type.simpleName == "String">
   ...
   public ${fieldSimple.name?uncap_first}: number = 0,
   </#if>
   <#if fieldSimple.type.simpleName == "Boolean">
   public ${fieldSimple.name?uncap_first}: null | boolean = null,
   </#if>
   <#if fieldSimple.type.simpleName == "Date" || fieldSimple.dateTime>
   public ${fieldSimple.name?uncap_first}: Date = new Date(),
   </#if>
   ...
   <#list pojo.fieldsGeneric as fieldGeneric>
   <#if fieldGeneric.pojo??>
   public ${fieldGeneric.name?uncap_first}: ${fieldGeneric.pojo.name}Dto = new ${fieldGeneric.pojo.name}Dto(),
   </#if>
   </#list>
   <#list pojo.fields as field>
   <#if field.list>
