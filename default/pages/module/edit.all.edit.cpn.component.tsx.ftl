        ...
        useEffect(() => {
              setItem(selectedItem ? { ...selectedItem } : { ...emptyItem });
               <#list pojo.fields as field>
                   ...
                   setItem((prevState) => ({
                     ...prevState,
                       ${field.name?uncap_first}:  selectedItem?.${field.name?uncap_first} ?? [],
                          }));
                            </#if>
                    </#list>
      ...
      <#list pojo.fields as field>
          ...
          const add${field.name?cap_first} = () => {
                   ...
                   if (!_item.id) {
                      ...
                      item.id === ${field.typeAsPojo.name?uncap_first}.id ? { ...item, <#if innerField.generic && innerField.typeAsPojo.name != pojo.name>${innerField.name?uncap_first}: { ...selected${innerField.name?cap_first} }<#assign i=0> <#list innerField.generic as innerField><#if innerField?index != innerField.generic?size -1>,</#if></#list>,${innerField.name?uncap_first}: ${field.typeAsPojo.name?uncap_first}.${innerField.name?uncap_first} } <#if innerField?index != field.typeAsPojo.fields?size -1>,</#if>: item,
                                    </#if>
                                </#if>
                            </#list>
                          );
        ...
        <div className="formgrid grid">
            ...
            <#elseif field.pureString>
                <div className="field col-6">
                    ...
                    <InputText id="${field.name?uncap_first}" value={item ? item.${field.name} : ''} onChange={(e) => onInputTextChange(e, '${field.name?uncap_first}')} required autoFocus
                    className={classNames({'p-invalid': submitted && !item.${field.name?uncap_first}})}/>
                    {submitted && !item.${field.name?uncap_first} &&
                    <small className="p-invalid">${field.name?cap_first} is required.</small>}
                </div>
                            ...
                            <#if  !innerField.notVisibleInCreatePage>
                                ...
                                <#elseif innerField.generic && innerField.typeAsPojo.name != pojo.name>
                                    <div className="field col-6">
                                        ...
                                        <Dropdown id="${innerField.name?uncap_first}Dropdown" value={selected${innerField.name?cap_first}} options={${innerField.name?uncap_first}s}
                                                  onChange={(e) => setSelected${innerField.name?cap_first}(e.value)} placeholder="Sélectionnez un ${field.name?uncap_first}" filter  filterPlaceholder="Rechercher un ${innerField.name?uncap_first}"  optionLabel="${innerField.typeAsPojo.labelOrReferenceOrId.name!'walo'}" />
                                    </div>
                        ...
                        <div className="card">
                            ...
                            <div>
                                <Button icon="pi pi-times" rounded severity="warning"
                                        ...
                                        onClick={() => delete${field.typeAsPojo.name}(rowData)}/>
                                <Button icon="pi pi-pencil" rounded severity="success" className="mr-2"
                                        onClick={() => edit${field.typeAsPojo.name}(rowData)}/>
                            </div>
                            )}></Column>
