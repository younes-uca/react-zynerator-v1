...
export const ${pojo.name}Service = {
   ...
   getList(): Promise<AxiosResponse<${pojo.name}[]>> {
     return axios.get(API_URL + '${pojo.name?uncap_first}/');
   },

   save(item: ${pojo.name}): Promise<AxiosResponse<${pojo.name}>> {
     return axios.post(API_URL + '${pojo.name?uncap_first}/', item);
   },

   update(item: ${pojo.name}): Promise<AxiosResponse<${pojo.name}>> {
      return axios.put(API_URL + '${pojo.name?uncap_first}/', item);
   },

   delete(id: number): Promise<AxiosResponse<${pojo.name}>> {
      return axios.delete(API_URL + '${pojo.name?uncap_first}/id/<#noparse>${id}</#noparse>');
   },

   deleteList(items: ${pojo.name}[]): Promise<AxiosResponse<string>> {
