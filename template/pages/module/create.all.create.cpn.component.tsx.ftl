...
const Create = ({visible, onClose, add, showToast, list}) => {
      ...
      const [${field.name}, set${field.name?cap_first}] = useState<${field.typeAsPojo.name}Dto[]>( []);
      ...
      const [selected${field.typeAsPojo.name?cap_first}, setSelected${field.typeAsPojo.name?cap_first}] = useState(null);
            </#if>
      </#list>
      ...
      const onDropdownChange = (e, field) => {
             set${pojo.name?cap_first}((prevState) => ({
                ...prevState,
                [field]: e.value,
             }));
      };
...
<#list pojo.fields as field>
      <#if field.list && !field.association>
          ...
          const onInputDateChange${field.name?cap_first} = (e: CalendarChangeEvent, name: string) => {
                 const val = e.value || ''; // Utilisez e.value au lieu de e.target.value
                  let _item = { ...${field.typeAsPojo.name}};
      ...
      const onInputTextChange${field.name?cap_first} = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, name: string) => {
       ...
       const saveItem = async () => {
                ...
                <#list pojo.fields as field>
                    <#if field.list && !field.association>
                 ${pojo.name?uncap_first}.${field.name?uncap_first} = ${field.name?uncap_first};
                    </#if>
                </#list>
                let _items = [...items];
                let _item = {...item};
    ...
    <div className="formgrid grid">
        ...
        <#elseif field.pureString>
            <div className="field col-6">
                ...
                <InputText id="${field.name?uncap_first}" value={item.${field.name?uncap_first}} onChange={(e) => onInputTextChange(e, '${field.name?uncap_first}')} required autoFocus
                    ...
                    <div className="card">
                        ...
                        <#list field.typeAsPojo.fields as myField>
                            <#if myField.simple && !myField.notVisibleInCreatePage>
                                <#if !myField.id>
                                    <Column field="prix" header="${myField.formatedName}"></Column>
                                </#if>
